...
Class Master extends DBConnection {
	...
	function save_shop_type(){
		...
		if($check > 0){
			$resp['status'] = 'failed';
			$resp['msg'] = "Business Type already exists.";
		}else{
			if(empty($id)){
			...
			if($save){
				$resp['status'] = 'success';
				if(empty($id))
				$resp['msg'] = " New Business Type successfully saved.";
				else
				$resp['msg'] = " Business Type successfully updated.";
			}else{
				$resp['status'] = 'failed';
	...
	function delete_shop_type(){
		...
		if($del){
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success'," Business Type successfully deleted.");
		}else{
			$resp['status'] = 'failed';
			$resp['error'] = $this->conn->error;
		}
	...
	function save_category(){
		...
		$check = $this->conn->query("SELECT * FROM `category_list` where `name` = '{$name}' and exporter_id = '{$exporter_id}' and delete_flag = 0 ".(!empty($id) ? " and id != {$id} " : "")." ")->num_rows;
		if($this->capture_err())
			return $this->capture_err();
		if($check > 0){
			$resp['status'] = 'failed';
	...
	function save_product(){
		...
		$check = $this->conn->query("SELECT * FROM `product_list` where exporter_id = '{$exporter_id}' and `name` = '{$name}' and delete_flag = 0 ".(!empty($id) ? " and id != '{$id}'" : ""))->num_rows;
		if($check > 0){
			$resp['status'] = 'failed';
	...
	function add_to_cart(){
		$_POST['importer_id'] = $this->settings->userdata('id');
		extract($_POST);
		$data = "";
		foreach($_POST as $k =>$v){
			if(!in_array($k,array('id'))){
		...
		$check = $this->conn->query("SELECT * FROM cart_list where product_id = '{$product_id}' && importer_id = '{$importer_id}'")->num_rows;
		if($check > 0){
			$sql = "UPDATE cart_list set quantity = quantity + {$quantity} where product_id = '{$product_id}' && importer_id = '{$importer_id}' ";
		}else{
	...
	function place_order(){
		...
		$has_failed=false;
		$gtotal = 0;
		$exporters = $this->conn->query("SELECT * FROM `exporter_list` where id in (SELECT exporter_id from product_list where id in (SELECT product_id FROM `cart_list` where importer_id ='{$this->settings->userdata('id')}')) order by `shop_name` asc");
		$prefix = date('Ym-');
		$code = sprintf("%'.05d",1);
		while($vrow = $exporters->fetch_assoc()):
			$data = "";
			while(true){
			...
			$sql = "INSERT INTO `order_list` (`code`,`importer_id`,`exporter_id`,`delivery_address`) VALUES {$data}";
			$save = $this->conn->query($sql);
			if($save){
				...
				$data = "";
				$gtotal = 0 ;
				$products = $this->conn->query("SELECT c.*, p.name as `name`, p.price,p.image_path FROM `cart_list` c inner join product_list p on c.product_id = p.id where c.importer_id = '{$this->settings->userdata('id')}' and p.exporter_id = '{$vrow['id']}' order by p.name asc");
		...
		if(!$has_failed){
			...
			$this->conn->query("DELETE FROM `cart_list` where importer_id ='{$this->settings->userdata('id')}'");
		}else{
			$resp['status'] = 'failed';
