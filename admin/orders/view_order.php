...
if(isset($_GET['id']) && $_GET['id'] > 0){
    $qry = $conn->query("SELECT o.*,c.code as ccode, CONCAT(c.lastname, ', ',c.firstname,' ',COALESCE(c.middlename,'')) as importer, CONCAT(v.code, '-',v.shop_name) as `exporter` from `order_list` o inner join importer_list c on o.exporter_id = c.id inner join exporter_list v on o.exporter_id = v.id where o.id = '{$_GET['id']}' ");
    if($qry->num_rows > 0){
...
<div class="container-fluid">
	<div class="row">
        ...
        <div class="col-3 border bg-gradient-primary"><span class="">exporter</span></div>
        <div class="col-9 border"><span class="font-weight-bolder"><?= isset($exporter) ? $exporter : '' ?></span></div>
        <div class="col-3 border bg-gradient-primary"><span class="">Importer</span></div>
        <div class="col-9 border"><span class="font-weight-bolder"><?= isset($importer) ? $ccode.' - '.$importer : '' ?></span></div>
