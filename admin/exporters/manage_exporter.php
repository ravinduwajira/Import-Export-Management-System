<?php 
if(isset($_GET['id'])){
	$exporter = $conn->query("SELECT * FROM exporter_list where id ='".$_GET['id']."'");
	if($exporter->num_rows > 0){
		$res = $exporter->fetch_array();
		foreach($res as $k =>$v){
			$$k = $v;
		}
	}else{
		echo '<script> alert("Unknown exporter"); location.replace("./?page=exporters")</script>';
	}
				...
				<form action="" id="manage-user">	
					...
					<div class="row">
						<div class="form-group col-md-6">
							<label for="shop_name" class="control-label">Business Name</label>
						...
						<div class="form-group col-md-6">
							<label for="shop_owner" class="control-label">Business Owner Fullname</label>
						...
						<div class="form-group col-md-6">
							<label for="shop_type_id" class="control-label">Business Type</label>
					...
					<div class="row">
						<div class="form-group col-md-6 text-center">
							<img src="<?= validate_image(isset($avatar) ? $avatar : "") ?>" alt="Business Logo" id="cimg" class="border border-gray img-thumbnail">
						</div>
					</div>
					<div class="row">
...
<script>
	...
	$(function(){
		...
		$('#manage-user').submit(function(e){
			...
			start_loader();
			$.ajax({
				url:_base_url_+"classes/Users.php?f=save_exporter",
