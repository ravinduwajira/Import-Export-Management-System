                    ...
                    <table class="table table-bordered table-stripped">
                        ...
                        <thead>
                            <tr class="">
                                ...
                                <th class="text-center align-middle py-1">Importer</th>
                                <th class="text-center align-middle py-1">exporter</th>
                                <th class="text-center align-middle py-1">Status</th>
                                <th class="text-center align-middle py-1">Total Amount (Rs.)</th>
                            </tr>
                        </thead>
                        <tbody>
                            <?php 
                            $i = 1;
                            $total = 0;
                            $orders = $conn->query("SELECT o.*,c.code as ccode, CONCAT(c.lastname, ', ',c.firstname,' ',COALESCE(c.middlename,'')) as importer,concat(v.code, '-',v.shop_name) as exporter from `order_list` o inner join importer_list c on o.importer_id = c.id inner join exporter_list v on o.exporter_id = v.id where date_format(o.date_created,'%Y-%m') = '{$month}' order by unix_timestamp(o.date_created) desc ");
                            ...
                            ?>
                                <tr>
                                    ...
                                    <td class="align-middle px-2 py-1"><?php echo ucwords($row['ccode'].' - '.$row['importer']) ?></td>
                                    <td class="align-middle px-2 py-1"><?php echo ucwords($row['exporter']) ?></td>
