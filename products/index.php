                    ...
                    <button type="submit" class="btn btn-primary">Apply Filter</button>
                        ...
                        <div class="row" id="product_list">
                            ...
                            if ($min_price !== '' && $max_price !== '') {
                                $swhere .= " AND p.price BETWEEN {$min_price} AND {$max_price}";
                            } elseif ($min_price !== '') {
                                $swhere .= " AND p.price >= {$min_price}";
                            } elseif ($max_price !== '') {
                                $swhere .= " AND p.price <= {$max_price}";
                            }
                            $products = $conn->query("SELECT p.*, v.shop_name as exporter, c.name as `category` FROM `product_list` p inner join exporter_list v on p.exporter_id = v.id inner join category_list c on p.category_id = c.id where p.delete_flag = 0 and p.`status` =1 {$swhere} order by RAND()");
